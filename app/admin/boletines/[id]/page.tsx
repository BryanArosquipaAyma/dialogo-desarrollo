...
export default function EditarBoletinPage() {
  ...
  useEffect(() => {
    fetch(`/api/boletines/${id}`)
      .then((r) => r.json())
      .then((data) => {
        if (data.boletin) {
          const b = data.boletin;
          setForm({
            numero_boletin: b.numero_boletin || "",
            titulo: b.titulo || "",
            resumen: b.resumen || "",
            foto_portada: b.foto_portada || "",
            archivo_pdf: b.archivo_pdf || "",
            fecha_publicacion: b.fecha_publicacion
              ? b.fecha_publicacion.slice(0, 10)
              : "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/boletines/${id}`, {
        method: "PUT",
      ...
      if (res.ok) {
        ...
        setTimeout(() => router.push("/admin/boletines"), 1000);
      } else {
  ...
  if (loading) {
    return <p className="text-gray-500">Cargando boletín...</p>;
  }

  return (
    <div className="mx-auto max-w-3xl">
      <h1 className="text-3xl font-bold text-gray-900">Editar boletín</h1>
